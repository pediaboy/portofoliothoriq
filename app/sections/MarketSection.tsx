...
function CandlestickChart() {
  ...
  useEffect(() => {
    ...
    candles.forEach((c, i) => {
      ...
      ctx.fillRect(x - cw * 0.3, Math.min(openY, closeY), cw * 0.6, bodyH)
    })
  }, [candles])
...
function LivePriceNumber({ base, change }: { base: number; change: number }) {
  const [val, setVal] = useState(base)
  useEffect(() => {
    const interval = setInterval(() => {
      setVal(prev => {
        const delta = (Math.random() - 0.5) * 20
        return Math.max(base * 0.95, Math.min(base * 1.05, prev + delta))
      })
    }, 1500)
    return () => clearInterval(interval)
  }, [base])
              ...
              <div className="ticker-inner animate-ticker">
                {[...tickers, ...tickers].map((t, i) => (
                  <div key={i} className="inline-flex items-center gap-2 px-6 border-r border-blue-900/30">
                    ...
                    <LivePriceNumber base={t.price} change={t.change} />
                ...
                {tickers.slice(0, 5).map(t => (
                  <div key={t.sym} className="flex items-center justify-between py-2 border-b border-blue-900/10 last:border-0">
                    ...
                    <span className={`text-sm font-bold px-2 py-0.5 rounded-lg ${
                      t.change >= 0 ? 'text-green-400 bg-green-400/10' : 'text-red-400 bg-red-400/10'
                    }`}>
                      {t.change >= 0 ? '+' : ''}{t.change}%
                    </span>
                  </div>
            ...
            <FadeIn delay={0.3}>
              <div className="market-card p-5">
                ...
                <div className="grid grid-cols-4 gap-1">
                  {tickers.map(t => (
                    <div key={t.sym} className="aspect-square rounded-lg flex flex-col items-center justify-center cursor-default"
                      style={{
                        background: t.change >= 0
                          ? `rgba(0,255,136,${Math.min(0.3, Math.abs(t.change) * 0.08)})`
                          : `rgba(255,68,102,${Math.min(0.3, Math.abs(t.change) * 0.08)})`,
                        border: `1px solid ${t.change >= 0 ? 'rgba(0,255,136,0.2)' : 'rgba(255,68,102,0.2)'}`,
                      }}>
                      <p className="text-xs font-bold text-white" style={{ fontSize: '9px' }}>{t.sym}</p>
                      <p className={`font-bold ${t.change >= 0 ? 'text-green-400' : 'text-red-400'}`} style={{ fontSize: '8px' }}>
                        {t.change >= 0 ? '+' : ''}{t.change}%
                      </p>
                    </div>
                  ))}
